<div id="sidebar"
    class="w-64 flex flex-col h-full bg-gradient-to-b from-gray-800 to-gray-900 text-white shadow-xl fixed inset-y-0 left-0 z-50 transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out md:relative md:transform-none">
    <!-- Header Image -->
            ...
            <ul class="space-y-2">
                ...
                <li>
                    ...
                    <a href="{{ route('notifications.index') }}"
                        class="flex items-center px-3 py-3 rounded-lg transition-all duration-200
                            {{ request()->routeIs('notifications.*') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                        <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <span class="font-medium flex-1 relative">Notifications
                            @if($unreadFollowUps > 0)
                                <span class="absolute -top-1 -right-4 inline-block w-3 h-3 bg-green-500 rounded-full border-2 border-white"></span>
                            @endif
                        </span>
                    </a>
                </li>
    ...
    <div class="p-4 border-t border-gray-700 bg-gray-800">
        ...
        <a href="{{ route('profile.show') }}"
            class="w-full flex items-center mb-4 px-3 py-3 rounded-lg text-gray-300 hover:bg-blue-900 hover:text-white transition-all duration-200">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit"
                class="w-full flex items-center px-3 py-3 rounded-lg text-gray-300 hover:bg-red-900 hover:text-white transition-all duration-200">
                <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<button id="sidebar-toggle" class="md:hidden fixed top-2 left-4 z-50 p-2  rounded-lg  focus:outline-none">
    ...
    <svg id="close-icon" class="w-6 h-6 hidden text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"
